...
- **Virtualize the long lists** — ~~match results, health-check issues and the duplicate pairs (top-200 cap stopgap) render every row~~ *(done 2026-07-13 — `src/ui/useVirtualList.ts`, a dependency-free windowing hook with measured row heights; only the on-screen rows mount, the duplicates cap is gone, and expanded-compare field choices survive the row scrolling out of the window)*. Still open: the **place/source trees** render every visible node — virtualizing them means flattening the recursive `tools-tree` markup first; worth it only if index-scale files make those panels hurt.

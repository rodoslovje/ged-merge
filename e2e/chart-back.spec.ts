...
// every person the user re-roots on (a node's "Root person", a "+N" cut marker,
...
test("Back walks back through the people a chart was re-rooted on", async ({ page }) => {
  ...
  // Re-root on another drawn person, twice, via the node panel's "Root person".
  const roots: string[] = [firstRoot];
  for (let i = 0; i < 2; i++) {
    const other = page.locator("svg.tree-svg g.tree-node").nth(i + 1);
    await other.click();
    await page.getByRole("button", { name: "Root person" }).click();

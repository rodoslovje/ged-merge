...
export function convertUpdToChan(records: GedNode[]): NormChange[] {
  ...
  for (const rec of records) {
    ...
    const upd = rec.children.find((c) => c.tag === "_UPD" && c.value?.trim());
    if (!upd || hasChild(rec, "CHAN")) continue;
    const m = UPD_RE.exec(upd.value!.trim());
    if (!m) continue;
    const [, date, time] = m;
    // Replace the _UPD node in place so the stamp keeps its position.
    upd.tag = "CHAN";
    upd.value = undefined;
    upd.children = [
      { level: 0, tag: "DATE", value: date.toUpperCase(), children: time ? [{ level: 0, tag: "TIME", value: time, children: [] }] : [] },
    ];
    changes.push({ before: `_UPD ${m[0]}`, after: `CHAN ${date.toUpperCase()}` });
  }
  return changes;
}
...
export function stripInternalTags(records: GedNode[]): NormChange[] {
  ...
  const strip = (node: GedNode): void => {
    ...
    for (const child of node.children) {
      if (vendorTagInfo(child.tag)?.category === "internal") {
        changes.push({ before: child.value?.trim() ? `${child.tag} ${child.value.trim()}` : child.tag, after: "(removed)" });
        continue;
      }
      strip(child);
      kept.push(child);

...
/**
 * Set (insert or replace) the `MAP` coordinates on a `PLAC` node. A new MAP
 * goes first among the PLAC's children after any `FORM`; an existing MAP has
 * its LATI/LONG rewritten in place.
 */
export function setPlaceCoord(placNode: GedNode, coord: GeoCoord): void {

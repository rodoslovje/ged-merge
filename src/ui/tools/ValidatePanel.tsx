...
/** Identity of one broken pointer (record + target), for the fixed-row set. */
function brokenLinkKey(issue: ValidationIssue): string {
  return `${issue.id}\0${brokenTarget(issue) ?? ""}`;
}
...
export function ValidatePanel({
  ...
  // Broken-link findings already repaired from their own row. The re-validate
  // they trigger runs in the worker, so the old list stays up for a moment —
  // these rows go quiet meanwhile instead of inviting a second, no-op click.
  const [fixedLinks, setFixedLinks] = useState<Set<string>>(() => new Set());
  ...
  // One finding's own repair: drop just the pointer that finding names. The
  // bulk button lives at the top of the panel, far from the row that raised the
  // question — and a broken link is invisible in Edit mode (a family that isn't
  // there draws nothing), so the finding itself has to carry the fix.
  function fixOneLink(issue: ValidationIssue) {
    onFixBrokenLinks({ id: issue.id, target: brokenTarget(issue) });
    setFixedLinks((prev) => new Set(prev).add(brokenLinkKey(issue)));
    scans.refresh("validate");
  }
  ...
  useEffect(() => {
    setFixedLinks((prev) => (prev.size === 0 ? prev : new Set()));
  }, [report]);
              ...
              {shown.slice(virtual.start, virtual.end).map((row, j) => {
                ...
                return row.kind === "record" ? (
                  <li key={`r-${row.issue.id}-${row.issue.category}-${i}`} className={`tools-issue sev-${row.issue.severity}${zebra}`}>
                    ...
                    {row.issue.category === "brokenLink" && (
                      fixedLinks.has(brokenLinkKey(row.issue)) ? (
                        <span className="tools-issue-fixed">{t("tools.validate.fixOneLinkDone")}</span>
                      ) : (
                        <button
                          className="tools-issue-fix"
                          title={t("tools.validate.fixOneLinkHint")}
                          onClick={() => fixOneLink(row.issue)}
                        >
                          {t("tools.validate.fixOneLink")}
                        </button>
                      )
                    )}
                  </li>

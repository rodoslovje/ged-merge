...
import { ageLabelFor, livingLabelFor, type NodeDisplayOptions } from "../chart/nodeDisplay";
...
export function useFanChart(
  ...
  const ageLabelOf = useCallback((n: TreeNode) => ageLabelFor(t, n.sex), [t]);
  const fan = useMemo(
    () => (tree ? buildFanChart(tree, shape, { hasPhoto, display, livingLabelOf, ageLabelOf, kinshipOf }) : undefined),
    [tree, shape, hasPhoto, display, livingLabelOf, ageLabelOf, kinshipOf],
  );
  const nodes = useMemo(() => {

...
- **Update the changelog on merge.** Before merging a new user-facing feature or fix to main, add a dated entry (today's date, newest entry at the top) to **both** `changelog/index.html` (English) and `posodobitve/index.html` (Slovenian) — same content in each — as one or two short, user-facing bullets (skip purely internal refactors/cleanup). Commit it in the worktree branch so it merges along with the feature.

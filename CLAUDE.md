...
- **Terminology: the UI wins, one concept = one name.** Grep `src/locales/sl.ts` for a term before writing it into a doc. Fixed choices: **diagram** (not *grafikon*), **mediji/medijski** (never *predstavnost*), **rod** (not *generacija*), **krogec** for map dots (the teardrop `PinIcon` alone is **bucika**, fem.), **stopnja ujemanja**, **prednastavitev**, **vhodna datoteka** (not *dohodna*), **izhodiščna oseba**, **hramba** for a GEDCOM REPO, **Pregled** for the health check. A UI chip is never *čip* (that is a microchip).
...
- **Register:** running prose addresses the reader with polite **vi**; button/menu labels are bare imperatives (*Odstrani*, *Izberi mapo*); never slip into *ti* forms (*tvoj*, *uporabiš*, *uvoziš*). Inclusive *mi* (»če ga stisnemo na A4«) is fine for didactic asides.
- **Punctuation:** quotes are »…«, thousands in map scales use a space (`1:75 000`), en-dash — with spaces for asides.
